...
let lambda_smaller' lam ~than:threshold =
  ...
  and lambda_named_size (named : Flambda.named) =
    if !size > threshold then raise Exit;
    match named with
    | Symbol _ | Read_mutable _ | Recursive _ -> ()
...
module Benefit = struct
  ...
  let evaluate t ~(args : Settings.Inlining.t) =
    benefit_factor *
      (t.remove_call * Settings.Inlining.inline_call_cost args
       + t.remove_alloc * Settings.Inlining.inline_alloc_cost args
       + t.remove_prim * Settings.Inlining.inline_prim_cost args
       + t.remove_branch * Settings.Inlining.inline_branch_cost args
       + (t.direct_call_of_indirect
         * Settings.Inlining.inline_indirect_cost args))
    + t.requested_inline
...
module Whether_sufficient_benefit = struct
  ...
  let estimated_benefit t =
    if t.toplevel && t.lifting && t.branch_depth = 0 then begin
      let lifting_benefit =
        t.args |> Settings.Inlining.inline_lifting_benefit
      in
      ...
      let branch_taken_estimated_probability =
        let inline_branch_factor =
          let factor =
            t.args |> Settings.Inlining.inline_branch_factor
          in
...
let maximum_interesting_size_of_function_body_base
      (max_args : Settings.Inlining.t) =
  (* because we are assuming monocity between rounds, the last
     round contains the maximum value *)
  direct_call_size + (Settings.Inlining.inline_call_cost max_args * benefit_factor)

let maximum_interesting_size_of_function_body_multiplier
      (max_args : Settings.Inlining.t) =
  Settings.Inlining.inline_prim_cost max_args * benefit_factor
