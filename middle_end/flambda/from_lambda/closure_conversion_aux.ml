...
module Acc = struct
  ...
  (* Executes [f] in an acc with an empty cost metrics and returns
     ...
     in the acc *)
  let with_blank_cost_metrics acc ~f =
...
module Expr_with_acc = struct
  ...
  let create_let (acc, let_expr) =
    (* The signature for create_let is a bit different. It is mainly used to
       materialize and expression coming from Let_cont_with_acc where the cost
       metrics was already computed. The signature is made so that the result
