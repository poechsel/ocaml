...
module rec Downwards_env : sig
  ...
  let print ppf { backend = _; round; typing_env; get_imported_code = _;
                  ...
                  inlining_state_increment; float_const_prop;
    ...
    Format.fprintf ppf "@[<hov 1>(\
        ...
        @[<hov 1>(can_inline@ %b)@]@ \
        @[<hov 1>(inlining_state_increment@ %d)@]@ \
  ...
  let get_inlining_state_increment t = t.inlining_state_increment

  let set_inlining_state_increment t inlining_state_increment =
    { t with inlining_state_increment; }
