...
module InliningArgs = struct
  ...
  let eq_u u1 u2 =
    u1.inline_call_cost = u2.inline_call_cost
    && u1.inline_alloc_cost = u2.inline_alloc_cost
    && u1.inline_prim_cost = u2.inline_prim_cost
    && u1.inline_branch_cost = u2.inline_branch_cost
    && u1.inline_indirect_cost = u2.inline_indirect_cost
    && u1.inline_lifting_benefit = u2.inline_lifting_benefit
    && Float.compare u1.inline_branch_factor u2.inline_branch_factor = 0
  ...
  the time the user will use one of the predefined levels (O1/O2/O3). *)
  type t =
      O1
    | O2
    | O3
  ...
  let convert_from_clflags (cl_var : Clflags.inlining_arguments) =
    ...
    in {
      ...
      inline_lifting_benefit =
        get Clflags.default_inline_lifting_benefit
            cl_var.inline_lifting_benefit;
      ...
      inline_max_speculation_depth =
        get Clflags.default_inline_max_speculation_depth
      ...
      inline_toplevel_threshold =
        get Clflags.default_inline_toplevel_threshold
            cl_var.inline_toplevel_threshold;
    }
  ...
  let get_inlining_arguments round =
    ...
    if eq_u theory o1_args then
      O1
    else if eq_u theory o2_args then
      O2
    else if eq_u theory o3_args then
      O3
    else
      Custom theory
  ...
  let merge_inlining_arguments args1 args2 =
    ...
    Custom {
      ...
      inline_branch_cost =
        min args1.inline_branch_cost args2.inline_branch_cost;
      inline_indirect_cost =
        min args1.inline_indirect_cost args2.inline_indirect_cost;
      inline_lifting_benefit =
        min args1.inline_lifting_benefit args2.inline_lifting_benefit;
      (* inline_branch_factor is strictly decreasing instead of increasing.*)
      inline_branch_factor =
        max args1.inline_branch_factor args2.inline_branch_factor;
      ...
      if round < Clflags.rounds () then begin
        ...
        let action format (label, proj, cmp, updater) =
          ...
          if (non_incr && cmp (proj previous) (proj current) < 0)
            || (not non_incr && cmp (proj previous) (proj current) > 0) then begin
...
let compare_rec_info rec_info1 rec_info2 =
  let compare_depth = compare rec_info1.depth rec_info2.depth in
  if compare_depth <> 0 then compare_depth
  else compare rec_info1.unroll_to rec_info2.unroll_to
  ...
  include Identifiable.Make (struct
    ...
    let compare (t1 : t) (t2 : t) =
      ...
      | Recursive (sym1, depth1), Recursive (sym2, depth2) ->
        ...
        if c <> 0 then c
        else compare_rec_info depth1 depth2
