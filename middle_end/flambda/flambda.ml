...
module InliningArgs = struct
  ...
  let cost ~round flag fn =
    match Clflags.Int_arg_helper.get ~key:round flag with
    | O1 -> fn o1_arguments
    | O2 -> fn o2_arguments
    | O3 -> fn o3_arguments
    | OClassic -> fn classic_arguments
    | Int x -> x
    | _ -> assert false

  let cost_f ~round flag fn =
    match Clflags.Float_arg_helper.get ~key:round flag with
    | O1 -> fn o1_arguments
    | O2 -> fn o2_arguments
    | O3 -> fn o3_arguments
    | OClassic -> fn classic_arguments
  ...
  let compress theory =
    if eq_u theory o1_arguments then
      O1
    else if eq_u theory o2_arguments then
      O2
    else if eq_u theory o3_arguments then
      O3
    else if eq_u theory classic_arguments then
      OClassic
    else
      Custom theory
           ...
           [("inline_branch_factor",
            ...
            (fun r x ->
               Clflags.inline_branch_factor :=
                 Clflags.Float_arg_helper.add_user_override r (Float x)
                   !Clflags.inline_branch_factor));]
        in
