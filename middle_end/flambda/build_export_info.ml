    ...
    *)
    let initial_value_set_of_closures =
      { Export_info.
        ...
        unboxing_arguments = set.unboxing_arguments;
  ...
  if !Clflags.opaque then
    ...
    let invariant_params =
      let rec symbol_to_closure_id sym =
        match Env.get_symbol_descr (Env.empty_of_global ~symbols_being_defined:(Symbol.Set.empty) env) sym with
      ...
      let invariant_params =
        Set_of_closures_id.Map.map
          (fun { Flambda. function_decls; _ } ->
             if Option.is_some function_decls.is_classic_mode then
             begin
               Variable.Map.empty
             end else begin
      ...
      function_declarations_map |> Set_of_closures_id.Map.filter_map
        (fun key (fun_decls : Simple_value_approx.function_declarations) ->
          if Set_of_closures_id.Set.mem key relevant_set_of_closures then
            ...
            if Option.is_some fun_decls.is_classic_mode then
