...
module InliningArgs : sig
  ...
  (** get an [inlining_arguments] struct filled with the
     ...
     As they are forced to increase between rounds, this is equivalent to
     getting the inlining arguments of the last round. *)
  val get : int -> t

  (* get an [inlining_arguments] struct filled with the
      maximum values across all round.
      As we are forcing them to be increasing over rounds, this is equivalent to
      be returning the arguments of the last round *)
  val get_max : unit -> t
