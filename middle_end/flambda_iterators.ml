          ...
          let funs =
            Variable.Map.map (fun (func_decl : Flambda.function_declaration) ->
                ...
                if new_body == func_decl.body then begin
                  ...
                  done_something := true;
                  Flambda.update_function_body func_decl new_body
                end)
              function_decls.funs
          in
          if not !done_something then
...
let map_symbols_on_set_of_closures
    ({ Flambda.function_decls; free_vars; specialised_args;
        direct_call_surrogates; rec_info; unboxing_arguments} as
      set_of_closures)
    ~f =
  let done_something = ref false in
  let funs =
    Variable.Map.map (fun (func_decl : Flambda.function_declaration) ->
        ...
        Flambda.update_function_body func_decl body)
      function_decls.funs
  in
  if not !done_something then
    set_of_closures
  ...
  let funs =
    Variable.Map.map (fun (function_decl : Flambda.function_declaration) ->
        ...
        if new_body == function_decl.body then
          ...
          done_something := true;
          Flambda.update_function_body function_decl new_body
        end)
      set_of_closures.function_decls.funs
  in
  if not !done_something then
