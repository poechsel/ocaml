...
let toplevel_substitution sb tree =
  ...
  let aux (flam : Flambda.t) : Flambda.t =
    ...
    | Assign { being_assigned; new_value; } ->
      ...
      Assign { being_assigned; new_value; }
    | Apply { func; args; kind; dbg; stack; inline; specialise; max_inlining_arguments} ->
      let func = sb func in
...
let substitute_read_symbol_field_for_variables
  ...
  let f (expr:Flambda.t) : Flambda.t =
    ...
    | Apply { func; args; kind; dbg; stack; inline; specialise; max_inlining_arguments } ->
      ...
      bind_func @@
      List.fold_right (fun f expr -> f expr) bind_args @@
      Flambda.Apply { func; args; kind; stack; dbg; inline; specialise; max_inlining_arguments }
    | Send { kind; meth; obj; args; dbg } ->
...
let empty_inlining_stack : Flambda.inlining_stack = []
...
let add_rec_info ?var named (rec_info : Flambda.rec_info) =
  ...
  | _ ->
    let tgt_var = match var with
      ...
      | None -> Variable.create "tgt"
    in
    let rec_var = match var with
      | Some var -> Variable.rename ~append:"_rec" var
      | None -> Variable.create "rec"
    in
