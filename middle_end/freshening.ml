        ...
        Variable.Map.map (fun (ffun : Flambda.function_declaration) ->
          let body =
            Flambda_iterators.map_toplevel_named
              ...
              ffun.body
          in
          Flambda.update_function_body ffun body)
  ...
  let func_decls_subst t (subst : subst)
    ...
    | Active subst ->
      let subst_func_decl _fun_id (func_decl : Flambda.function_declaration)
        ...
        in
        let function_decl =
          Flambda.create_function_declaration ~recursive:func_decl.recursive
            ~params ~body ~stub:func_decl.stub ~dbg:func_decl.dbg
