...
and simplify_set_of_closures original_env r
  ...
  let simplify_function fun_var (function_decl : Flambda.function_declaration)
    ...
    let inlining_history =
      Inlining_history.History.add function_decl.inlining_history parts_inlining_stack
    in
    let (body, r) =
      let body_env =
      ...
      in
      begin if not function_decl.stub then E.record_definition body_env end;
...
let run ~never_inline ~backend ~round program =
  let r = R.create () in
  let report = Clflags.backup_inlining_report () in
  if never_inline then Clflags.turn_inlining_report_off ();
  let initial_env =
  ...
  Clflags.restore_inlining_report report;
  result
