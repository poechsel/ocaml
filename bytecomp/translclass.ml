  ...
  if params = [] then body else
    ...
    |  _ ->
      Lfunction {kind = Curried; params;
                 ...
                 debugging_informations = dbg;
                 loc = Location.none;
                }

let lfunction_debug dbg params body =
  lfunction' dbg params body

let lfunction modpath loc name params body =
  let dbg = IH.add_fn_def ~name ~path:modpath ~loc in
  lfunction' dbg params body
...
let set_inst_var modpath obj id expr =
  Lprim(Psetfield_computed (Typeopt.maybe_pointer expr, Assignment),
    [Lvar obj; Lvar id; transl_exp modpath IH.Anonymous expr], Location.none)

let transl_val modpath tbl create name =
  mkappl modpath (oo_prim (if create then "new_variable" else "get_variable"),
          [Lvar tbl; transl_label name])

let transl_vals modpath tbl create strict vals rem =
  List.fold_right
    (fun (name, id) rem ->
      Llet(strict, Pgenval, id, transl_val modpath tbl create name, rem))
    vals rem

let meths_super modpath tbl meths inh_meths =
  List.fold_right
    (fun (nm, id) rem ->
       try
         (nm, id,
...
let bind_super modpath tbl (vals, meths) cl_init =
  transl_vals modpath tbl false StrictOpt vals
    (List.fold_right (fun (_nm, id, def) rem ->
         Llet(StrictOpt, Pgenval, id, def, rem))
       meths cl_init)

let create_object modpath cl obj init =
  ...
  if obj_init = lambda_unit then
   ...
   (inh_init,
    Llet(Strict, Pgenval, obj',
            mkappl modpath (oo_prim "create_object_opt", [obj; Lvar cl]),
         Lsequence(obj_init,
                   if not has_init then Lvar obj' else
                   mkappl modpath (oo_prim "run_initializers_opt",
  ...
  match cl.cl_desc with
    Tcl_ident ( path, _, _) ->
      ...
      ((envs, (obj_init, normalize_cl_path cl path)
        ::inh_init),
       mkappl modpath (Lvar obj_init, env @ [obj]))
  | Tcl_structure str ->
      create_object modpath cl_table obj (fun obj modpath ->
        ...
        (inh_init,
         List.fold_right
           (fun (id, expr) rem ->
              lsequence (Lifused (id, set_inst_var modpath obj id expr)) rem)
           params obj_init,
         has_init))
  | Tcl_fun (_, pat, vals, cl, partial) ->
      ...
      (inh_init,
       let build params rem =
         ...
         let name = IH.Class(Ident.name cl_id,
                                     IH.ObjInit)
         in
         Lfunction {kind = Curried; params = param::params;
                    ...
                    loc = pat.pat_loc;
                    debugging_informations =
                      IH.add_fn_def ~name ~path:modpath ~loc:pat.pat_loc;
  ...
  | _ ->
      ...
      let obj_init =
        if ids = [] then
          ...
          lfunction modpath cl.cl_loc name [self] obj_init
      in
      (inh_init,
       lfunction modpath cl.cl_loc name [env] (subst_env env inh_init obj_init))


let bind_method modpath tbl lab id cl_init =
  Llet(Strict, Pgenval, id, mkappl modpath (oo_prim "get_method_label",
...
let rec build_class_init modpath cl_id cla cstr super
  ...
  | Tcl_structure str ->
      let cl_init = bind_super modpath cla super cl_init in
      let (inh_init, cl_init, methods, values) =
        List.fold_right
          (fun field (inh_init, cl_init, methods, values) ->
            match field.cf_desc with
              Tcf_inherit (_, cl, _, vals, meths) ->
                let cl_init = output_methods modpath cla methods cl_init in
                let inh_init, cl_init =
                  build_class_init modpath cl_id cla false
                    (vals, meths_super modpath cla str.cstr_meths meths)
            ...
            | Tcf_method (name, _, Tcfk_concrete (_, exp)) ->
              let met_name = IH.Method(Ident.name cl_id, name.txt) in
              let met_code = msubst true (transl_exp modpath met_name exp) in
              let met_code =
              ...
              (inh_init, cl_init,
               ...
               values)
            | Tcf_initializer exp ->
                let exp = transl_exp modpath IH.Anonymous exp in
                (inh_init,
                 Lsequence(mkappl modpath (oo_prim "add_initializer",
        ...
        Tcl_ident (path, _, _), (obj_init, path')::inh_init ->
          ...
          (inh_init,
           Llet (Strict, Pgenval, inh,
                 mkappl modpath (oo_prim "inherits", narrow_args @
...
let rec build_class_lets modpath cl =
  match cl.cl_desc with
    Tcl_let (rec_flag, defs, _vals, cl') ->
      let env, wrap = build_class_lets modpath cl' in
      (env, fun x ->
          Translcore.transl_let modpath rec_flag defs (wrap x))
  | _ ->
      (cl.cl_env, fun x -> x)
...
(*
   ...
   |   Writing classes should be cheap
     class c x y = d e f
*)
let rec transl_class_rebind modpath cl_id obj_init cl vf =
  match cl.cl_desc with
    Tcl_ident (path, _, _) ->
  ...
  | Tcl_fun (_, pat, _, cl, partial) ->
      ...
      let build params rem =
        ...
        Lfunction {kind = Curried; params = param::params;
                   ...
                   loc = pat.pat_loc;
                   debugging_informations =
                     IH.add_fn_def ~name:d_name ~path:modpath ~loc:pat.pat_loc;
                   body = Matching.for_function
                            pat.pat_loc None (Lvar param) [pat, rem] partial}
      in
      (path,
  ...
  | Tcl_apply (cl, oexprs) ->
      ...
      (path, transl_apply modpath obj_init oexprs Location.none)
  | Tcl_let (rec_flag, defs, _vals, cl) ->
      let path, obj_init = transl_class_rebind modpath cl_id obj_init cl vf in
      (path, Translcore.transl_let modpath rec_flag defs obj_init)
  ...
  | Tcl_constraint (cl', _, _, _, _) ->
      ...
      (path, obj_init)
  | Tcl_open (_, _, _, _, cl) ->
      transl_class_rebind modpath cl_id obj_init cl vf

let rec transl_class_rebind_0 modpath cl_id self obj_init cl vf =
  match cl.cl_desc with
    Tcl_let (rec_flag, defs, _vals, cl) ->
    let path, obj_init = transl_class_rebind_0 modpath
  ...
  | _ ->
    ...
    in
      (path, lfunction modpath cl.cl_loc d_name [self] obj_init)
...
let transl_class_rebind modpath cl_id cl vf =
  try
    ...
    in
    let obj_init0_fn =
      lfunction modpath cl.cl_loc IH.Anonymous [self] obj_init0
    in
...
let transl_class modpath ids cl_id pub_meths cl vflag =
  (* First check if it is not only a rebind *)
  let rebind = transl_class_rebind modpath cl_id cl vflag in
  ...
  let top = not req in
  let cl_env, llets = build_class_lets modpath cl in
  ...
  and ldirect obj_init =
    Llet(Strict, Pgenval, obj_init, cl_init,
         Lsequence(mkappl modpath (oo_prim "init_class", [Lvar cla]),
                   mkappl modpath (Lvar obj_init, [lambda_unit])))
  in
  ...
  and lclass lam =
    let cl_name =
      IH.Class(Ident.name cl_id, IH.ClassInit)
    in
    let debug = IH.add_fn_def ~name:cl_name ~path:modpath ~loc:cl.cl_loc in
  ...
  and lbody fv =
    if List.for_all (fun id -> not (Ident.Set.mem id fv)) ids then
      mkappl modpath (oo_prim "make_class",[transl_meth_list pub_meths;
                                    Lvar class_init])
    else
      ltable table (
      Llet(
      ...
      Lprim(Pmakeblock(0, Immutable, None),
            [mkappl modpath (Lvar env_init, [lambda_unit]);
  ...
  and lbody_virt lenvs =
    Lprim(Pmakeblock(0, Immutable, None),
          [lambda_unit; Lfunction{kind = Curried;
                                  debugging_informations =
                                    IH.add_fn_def ~path:modpath
                                      ~name:IH.Anonymous ~loc:cl.cl_loc;
                                  attr = default_function_attribute;
                                  loc = Location.none;
  ...
  let make_envs lam =
    Llet(StrictOpt, Pgenval, envs,
         ...
         lam)
  and def_ids modpath cla lam =
    Llet(StrictOpt, Pgenval, env2,
         mkappl modpath (oo_prim "new_variable", [Lvar cla; transl_label ""]),
         lam)
  in
  let inh_paths =
    List.filter
  ...
  let lclass lam =
    ...
    in
    let debug =
      IH.add_fn_def ~name:cl_name ~path:modpath ~loc:cl.cl_loc
    in
  ...
  and lcache lam =
    ...
    Llet(Strict, Pgenval, cached,
         mkappl modpath (oo_prim "lookup_tables",
  ...
  let ldirect () =
    ltable cla
      (Llet(Strict, Pgenval, env_init, def_ids modpath cla cl_init,
            Lsequence(mkappl modpath (oo_prim "init_class", [Lvar cla]),
  ...
  let lupdate_cache =
    if ids = [] then ldirect () else
      if not concrete then lclass_virt () else
        lclass (
            mkappl modpath (oo_prim "make_class_store",
  ...
  make_envs (
  if ids = [] then mkappl modpath (lfield cached 0, [lenvs]) else
  Lprim(Pmakeblock(0, Immutable, None),
        (if concrete then
          [mkappl modpath (lfield cached 0, [lenvs]);
           lfield cached 1;
           lfield cached 0;
           lenvs]
...
let transl_class modpath ids id pub_meths cl vf =
  oo_wrap cl.cl_env false (transl_class modpath ids id pub_meths cl) vf

let () =
  transl_object := (fun modpath id meths cl -> transl_class modpath [] id meths cl Concrete)
